...
#         pos_weight = torch.tensor(min(class_weights[1] / class_weights[0], 20.0), dtype=torch.float32).to(device)
...
#         criterion = nn.MSELoss()  # unchanged
#     else:
...
def ddp_train(rank, world_size, model_name, data_path, epochs, lr, batch_size):
    ...
    model.to(device)
    model = nn.parallel.DistributedDataParallel(model, device_ids=[rank] if backend == "nccl" else None)
    optimizer = torch.optim.Adam(model.parameters(), lr=lr)
    ...
    if rank == 0:
        ...
        if model_name == "autoencoder":
            logger.info("Evaluating autoencoder as anomaly detector (rank 0)")
            full_train_loader = DataLoader(train_loader.dataset, batch_size=batch_size, shuffle=False)
            train_errors = []
            with torch.no_grad():
                for Xb, _ in full_train_loader:
                    Xb = Xb.to(device)
...
def train(model_name="dnn", data_path="data.csv", epochs=10, lr=1e-3, batch_size=64, world_size=2):
    logger = get_logger("distributed")
    os.environ["MASTER_ADDR"] = "127.0.0.1"
    if "MASTER_PORT" not in os.environ:
        os.environ["MASTER_PORT"] = str(29500 + random.randint(1, 1000))
